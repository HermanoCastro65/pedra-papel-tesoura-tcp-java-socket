	...
	private class Jogo {
		...
		private void aguardarJogadores() throws IOException {
			...
			while (numJogadores < 2) {
				...
				var out = new PrintStream(clientSocket.getOutputStream());//BufferedWriter(new OutputStreamWriter(clienteSocket.getOutputStream()));
		...
		private void obterPontuacao() {
			for(var jogador : TCPServer.this.jogadores) {
				...
				if (pontuacao != null) {
					...
					String msg = String.format("pontuacao: O jogador '%s' nao possui pontuação registrada", jogador.getNome());
